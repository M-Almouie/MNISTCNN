...
def my_relu(in_value):
    ...
    return tf.maximum(0.0, in_value)


def my_perceptron(x):
    """
    Implement a single perceptron that takes four inputs and produces one output,
    ...
    weights = (tf.get_variable(name="weights",
                               ...
                               trainable=True))
    product = tf.multiply(i, weights)
    total = tf.reduce_sum(product)
    out = my_relu(total)
...
def onelayer(X, Y, layersize=10):
    ...
    """
    # Set the weights/bias, both initialised to 0
    #w = tf.Variable(tf.random_normal(shape=[784, layersize], stddev=0.01))
    w = tf.Variable(tf.zeros([784, layersize]))
    b = tf.Variable(tf.zeros([1, layersize]))
    
    # Input for activation function  |  Y = mX + b
    logits = tf.matmul(X, w) + b
    
    # Use softmax to normalise
    preds = tf.nn.softmax(logits)
    
    # Loss function
    # Use softmax with cross-entropy. Logits is used as the loss function
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    """
    # Weights and biases
    w1 = tf.Variable(tf.random_normal([784, hiddensize], stddev=0.01))
    b1 = tf.Variable(tf.random_normal([hiddensize]))
    w2 = tf.Variable(tf.random_normal([hiddensize, outputsize], stddev=0.01))
    b2 = tf.Variable(tf.random_normal([outputsize]))
    
    # Hidden layer initialised
    hidden1 = tf.add(tf.matmul(X, w1), b1)
    
    # Activation function input (ReLU on hidden layer)
    logits = tf.add(tf.matmul(tf.nn.relu(hidden1), w2), b2)
    
    # Softmax to yield normalised output
    preds = tf.nn.softmax(logits)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    strides = 1  # For conv2d()
    x = tf.reshape(X, shape=[-1, 28, 28, 1])  # Batch Size, Height, Width, Channel
    
    # Weights and biases (conv1, conv2, connected, final)
    ...
    w = tf.Variable(tf.random_normal([1024, outputsize]))
    b = tf.Variable(tf.random_normal([outputsize]))
    
    # First convolutional layer
    conv1 = tf.nn.conv2d(x, w1, strides=[1, strides, strides, 1], padding="SAME")
    ...
    full = tf.reshape(conv2, [-1, w3.get_shape().as_list()[0]])
    full = tf.add(tf.matmul(full, w3), b3)
    full = tf.nn.relu(full)
    
    # Output layer
    logits = tf.add(tf.matmul(full, w), b)
